...
export async function POST(
  ...
  try {
    ...
    if (!payload || payload.role !== 'PETUGAS_SPT') {
      return NextResponse.json({ error: 'PETUGAS_SPT access required' }, { status: 403 })
    }

    const { notes, internalNotes, serviceCategory, priorityLevel, serviceDuration } = await request.json()
    ...
    // Check if this queue is part of a "BOTH" service sequence
    if (queue.serviceOrder && !queue.isSequenceComplete) {
      // Determine if this was the first or second service in the sequence
      const wasFirstService = (
        (queue.serviceType === 'HELPDESK' && queue.serviceOrder === 'HELPDESK_FIRST') ||
        (queue.serviceType === 'TPT' && queue.serviceOrder === 'TPT_FIRST')
      )
      
      if (wasFirstService) {
        // This was the first service, switch to the second service
        const nextServiceType = queue.serviceOrder === 'HELPDESK_FIRST' ? 'TPT' : 'HELPDESK'
        
        // Update the queue to continue with the next service
        await prisma.queue.update({
          where: { id: queueId },
          data: {
            serviceType: nextServiceType,
            status: 'WAITING', // Reset to waiting for next service
            startedAt: null, // Reset start time
            calledAt: null, // Reset call time
            calledBy: null, // Reset called by
            updatedAt: new Date()
          }
        })
        
        return NextResponse.json({
          queue: { ...queue, serviceType: nextServiceType, status: 'WAITING' },
          nextService: nextServiceType,
          message: `Antrian dilanjutkan ke layanan ${nextServiceType === 'HELPDESK' ? 'Helpdesk' : 'TPT'}`
        })
      } else {
        // This was the second service, mark sequence as complete
        await prisma.queue.update({
          where: { id: queueId },
          data: {
