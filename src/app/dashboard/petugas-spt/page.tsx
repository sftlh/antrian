...
export default function PetugasSPTDashboard() {
  ...
  useEffect(() => {
    const savedServiceCompleted = localStorage.getItem('spt_service_completed')
    if (savedServiceCompleted === 'true') {
      setServiceCompleted(true)
    }

    const savedCurrentQueue = localStorage.getItem('spt_current_queue')
    ...
    const savedServiceNotes = localStorage.getItem('spt_service_notes')
    if (savedServiceNotes) {
      setServiceNotes(savedServiceNotes)
    }

    const savedInternalNotes = localStorage.getItem('spt_internal_notes')
    if (savedInternalNotes) {
      setInternalNotes(savedInternalNotes)
    }

    const savedServiceCategory = localStorage.getItem('spt_service_category')
    if (savedServiceCategory) {
      setServiceCategory(savedServiceCategory)
    }

    const savedPriorityLevel = localStorage.getItem('spt_priority_level')
    if (savedPriorityLevel) {
      setPriorityLevel(savedPriorityLevel)
    }

    const savedServiceStartTime = localStorage.getItem('spt_service_start_time')
  ...
  useEffect(() => {
    localStorage.setItem('spt_service_completed', serviceCompleted.toString())
  }, [serviceCompleted])

  useEffect(() => {
    if (currentQueue) {
      localStorage.setItem('spt_current_queue', JSON.stringify(currentQueue))
    } else {
      localStorage.removeItem('spt_current_queue')
    }
  }, [currentQueue])

  useEffect(() => {
    localStorage.setItem('spt_service_notes', serviceNotes)
  }, [serviceNotes])

  useEffect(() => {
    localStorage.setItem('spt_internal_notes', internalNotes)
  }, [internalNotes])

  useEffect(() => {
    localStorage.setItem('spt_service_category', serviceCategory)
  }, [serviceCategory])

  useEffect(() => {
    localStorage.setItem('spt_priority_level', priorityLevel)
  }, [priorityLevel])

  useEffect(() => {
    if (serviceStartTime) {
      localStorage.setItem('spt_service_start_time', serviceStartTime.toISOString())
    } else {
      localStorage.removeItem('spt_service_start_time')
    }
  }, [serviceStartTime])
  ...
  const loadDailyStats = async () => {
    try {
      ...
      const response = await fetch('/api/dashboard/petugas-spt/stats', {
        headers: {
  ...
  const loadCustomLocation = () => {
    const saved = localStorage.getItem('spt_custom_location')
    if (saved) {
  ...
  const saveCustomLocation = (location: string) => {
    ...
    localStorage.setItem('spt_custom_location', location)
  }
  ...
  }, [user, isLoading, router, loadAudioDevices, filterType])

  // Clear localStorage when user logs out
  useEffect(() => {
    if (!user && !isLoading) {
      localStorage.removeItem('spt_service_completed')
      localStorage.removeItem('spt_current_queue')
      localStorage.removeItem('spt_service_notes')
      localStorage.removeItem('spt_internal_notes')
      localStorage.removeItem('spt_service_category')
      localStorage.removeItem('spt_priority_level')
      localStorage.removeItem('spt_service_start_time')
    }
  }, [user, isLoading])
  ...
  const fetchQueues = async () => {
    try {
      const token = localStorage.getItem('auth_token')
      const response = await fetch(`/api/queues/petugas-spt?type=${filterType}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const callNextCustomer = () => {
    ...
    if (nextQueue) {
      ...
      localStorage.setItem('spt_service_completed', 'false')
  ...
  const completeService = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token')
      const response = await fetch(`/api/queues/petugas-spt/${currentQueue.id}/complete`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('spt_current_queue', JSON.stringify(currentQueue))
        localStorage.setItem('spt_service_completed', 'true')
  ...
  const escalateService = async (reason: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/dashboard/petugas-spt/escalate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const forceCompleteQueue = async (queueId: string) => {
    ...
    try {
      const token = localStorage.getItem('auth_token')
      const response = await fetch(`/api/queues/petugas-spt/${queueId}/complete`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-3xl font-bold text-gray-900">Dashboard Petugas SPT</h1>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white shadow rounded-lg p-6 mb-6">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-semibold">Filter Layanan SPT</h2>
                  <select
                    value={filterType}
                    onChange={(e) => setFilterType(e.target.value)}
                    className="block w-64 pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-orange-500 focus:border-orange-500 sm:text-sm rounded-md"
                  >
                    <option value="BOTH">Tampilkan Semua (OP & Badan)</option>
                    <option value="OP">SPT Tahunan OP Saja</option>
                    <option value="BADAN">SPT Tahunan Badan Saja</option>
                  </select>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full border-gray-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-sm"
                    >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {currentQueue ? (
                  <div>
                    <div className="text-4xl font-bold text-center text-orange-600 mb-4">
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <div className="flex space-x-3">
                ...
                <button
                  onClick={() => setShowBreakReminder(false)}
                  className="px-4 py-2 border border-gray-300 rounded hover:bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50"
                >
                  Nanti saja
                </button>
              </div>
